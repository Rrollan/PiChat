...
const defaultSettings: RuntimeSettings = {
  piPath: 'pi', startupDirectory: '', piConfigDirectory: '', hiddenModelKeys: [], browserExtensionId: '', cliNoSession: true,
...
export default function App() {
  ...
  const sendMessage = async () => {
    ...
    try { await window.pichat.prompt(promptText, images); } catch (e: any) { setWaiting(false); notify(e?.message || String(e), 'error'); setMessages(prev => [...prev, systemMessage(`❌ ${e?.message || e}`)]); }
  };
  ...
  const connectedModels = useMemo(() => {
    const providers = new Set([...connectedProviders, ...config.authEntries.map(e => e.provider.toLowerCase()), ...(currentModel ? [currentModel.provider.toLowerCase()] : [])].map(p => p.replaceAll('_', '-')));
  ...
  }, [availableModels, connectedProviders, config.authEntries, currentModel]);
